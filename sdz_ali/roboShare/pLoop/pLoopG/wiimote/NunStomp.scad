...
//Z=32;
frontX=100; //can't be changed
frontZ=25; //can't be changed
...
translate([frameW+(X-frameW*4)/4-pcbMdist/2,Ya/2+pcbJdist,0])cylinder(d=scrwDia*2.5, h=pcb2bottom,$fn=12);
translate([frameW+(X-frameW*4)/4+pcbMdist/2,Ya/2+pcbJdist,0])cylinder(d=scrwDia*2.5, h=pcb2bottom,$fn=12);
}
translate([frameW+(X-frameW*4)/4-pcbMdist/2,Ya/2+pcbJdist,0])cylinder(d=scrwDia-t, h=pcb2bottom,$fn=12);
translate([frameW+(X-frameW*4)/4+pcbMdist/2,Ya/2+pcbJdist,0])cylinder(d=scrwDia-t, h=pcb2bottom,$fn=12);
...
translate([frameW+(X-frameW*4)/4,y/2,zPos-potZ/2])cylinder(h=10,d=20);
translate([frameW*3+(X-frameW*4)/4*3,y/2,zPos-potZ/2])cylinder(h=10,d=arcBtnD);
...
difference(){
rotate([0,0,0])linear_extrude(height=middleX, convexity= 100)
    polygon(points = [ [0,0],[length,0],[length,wallkneeA],[wallkneeB,height],[0,height] ], convexity = 1);
...
//translate([length-topAspZ,Ya,0])rotate([0,0,-40])cube([length,Ya/3,width*2]);
union(){
translate([length-5,wallkneeA/2+screwDist/2,width])rotate([0,90,0])cylinder(d=scrwDia-t, h=screwZ,$fn=12);
translate([length-5,wallkneeA/2-screwDist/2,width])rotate([0,90,0])cylinder(d=scrwDia-t, h=screwZ,$fn=12);
translate([length/2,10,width])rotate([90,0,0])cylinder(d=scrwDia-t, h=screwZ,$fn=12);
}
}}
...
difference(){
union(){
translate([0,0,-4])cylinder(h=4,d=32);
cylinder(h=6,d=12);}
translate([0,0,0])cylinder(h=6,d1=1,d2=8);
}}

translate([-frontSp,0,0]){
//baseStl();
